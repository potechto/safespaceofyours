-- V42 Public Space comment replies
-- Rules:
-- 1. One top-level comment per user per post.
-- 2. One reply per user per top-level comment.
-- 3. Replies are editable within the same 30-minute window.
-- 4. Reply notifications go to the comment owner.
...
begin
  ...
  return coalesce(
    (
      ...
      left join public.public_space_comments parent on parent.id = c.parent_comment_id
      where c.post_id = input_post_id
        ...
        and (
          ...
          or exists (
            select 1
            from public.public_space_comments parent_check
            where parent_check.id = c.parent_comment_id
              and parent_check.is_deleted is false
              and (
                parent_check.is_hidden is false
                or coalesce(active_user.is_admin, false)
              )
          )
        )
...
select
  'v42_public_space_comment_replies_ready' as status,

-- V35 Public Space registered users polish
-- Fixes old badge_label check constraint and makes Premium badge drive premium access.

begin;

alter table public.public_space_users
  drop constraint if exists public_space_users_badge_label_check;

alter table public.public_space_users
  add constraint public_space_users_badge_label_check
  check (
    badge_label is null
    or char_length(trim(badge_label)) <= 120
  );

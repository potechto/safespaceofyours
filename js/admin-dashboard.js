...
/* V2.0Q.16 admin code info floating portal */
(function setupAdminCodeInfoFloatingPortal() {
  if (window.__safeAdminCodeInfoFloatingPortalBound) return;
  window.__safeAdminCodeInfoFloatingPortalBound = true;

  let activeCodeInfoTrigger = null;
  let floatingCodeInfoPopover = null;

  function ensureFloatingCodeInfoPopover() {
    if (floatingCodeInfoPopover) return floatingCodeInfoPopover;

    floatingCodeInfoPopover = document.createElement("div");
    floatingCodeInfoPopover.id = "adminCodeFloatingPopover";
    floatingCodeInfoPopover.className = "admin-code-popover admin-code-floating-popover";
    floatingCodeInfoPopover.setAttribute("role", "note");
    floatingCodeInfoPopover.hidden = true;
    document.body.appendChild(floatingCodeInfoPopover);

    floatingCodeInfoPopover.addEventListener("pointerleave", event => {
      const nextTarget = event.relatedTarget;
      const activeWrap = activeCodeInfoTrigger?.closest(".admin-code-info-wrap");

      if (activeWrap && nextTarget instanceof Node && activeWrap.contains(nextTarget)) return;
      closeFloatingCodeInfoPopover();
    });

    return floatingCodeInfoPopover;
  }

  function getCodeInfoSourcePopover(trigger) {
    return trigger
      ?.closest(".admin-code-info-wrap")
      ?.querySelector(".admin-code-popover:not(.admin-code-floating-popover)");
  }

  function clearCodeInfoTriggerState() {
    document.querySelectorAll("[data-code-info-touch][aria-expanded='true']").forEach(button => {
      button.setAttribute("aria-expanded", "false");
    });
  }

  function positionFloatingCodeInfoPopover(trigger) {
    const popover = ensureFloatingCodeInfoPopover();
  ...
  function openFloatingCodeInfoPopover(trigger) {
    const sourcePopover = getCodeInfoSourcePopover(trigger);
    if (!sourcePopover) return;

    const popover = ensureFloatingCodeInfoPopover();
    activeCodeInfoTrigger = trigger;
    clearCodeInfoTriggerState();
    trigger.setAttribute("aria-expanded", "true");
    popover.innerHTML = sourcePopover.innerHTML;
    popover.hidden = false;
    popover.classList.add("is-visible");

    window.requestAnimationFrame(() => {
      if (activeCodeInfoTrigger === trigger) {
        positionFloatingCodeInfoPopover(trigger);
      }
    });
  }

  function closeFloatingCodeInfoPopover() {
    if (floatingCodeInfoPopover) {
      floatingCodeInfoPopover.hidden = true;
      floatingCodeInfoPopover.classList.remove("is-visible");
      floatingCodeInfoPopover.innerHTML = "";
    }

    clearCodeInfoTriggerState();
    activeCodeInfoTrigger = null;
  }

  function shouldKeepCodeInfoOpen(nextTarget) {
    if (!(nextTarget instanceof Node)) return false;
    const activeWrap = activeCodeInfoTrigger?.closest(".admin-code-info-wrap");
    return Boolean(
      activeWrap?.contains(nextTarget) ||
      floatingCodeInfoPopover?.contains(nextTarget)
    );
  }

  document.addEventListener("pointerover", event => {
    const trigger = event.target.closest("[data-code-info-touch]");
    if (!trigger || event.pointerType === "touch") return;
    openFloatingCodeInfoPopover(trigger);
  }, true);
  ...
  document.addEventListener("pointerdown", event => {
    const trigger = event.target.closest("[data-code-info-touch]");
    if (!trigger || event.pointerType === "mouse") return;
    openFloatingCodeInfoPopover(trigger);
  }, true);

  document.addEventListener("pointerout", event => {
    if (!activeCodeInfoTrigger) return;
    const activeWrap = activeCodeInfoTrigger.closest(".admin-code-info-wrap");
    if (!activeWrap?.contains(event.target)) return;
    if (shouldKeepCodeInfoOpen(event.relatedTarget)) return;
    closeFloatingCodeInfoPopover();
  }, true);
  ...
  document.addEventListener("focusout", () => {
    window.setTimeout(() => {
      if (!activeCodeInfoTrigger) return;
      if (document.activeElement === activeCodeInfoTrigger) return;
      if (floatingCodeInfoPopover?.contains(document.activeElement)) return;
      closeFloatingCodeInfoPopover();
    }, 0);
  }, true);
  ...
  document.addEventListener("pointercancel", closeFloatingCodeInfoPopover, true);
  document.addEventListener("scroll", closeFloatingCodeInfoPopover, true);
  window.addEventListener("resize", closeFloatingCodeInfoPopover);
...
/* V2.0Q.16 admin code info floating portal END */
